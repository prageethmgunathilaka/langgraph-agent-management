...
from app.models.schemas import AgentCreate, AgentStatusUpdate, AgentStatusEnum, WorkflowCreate, LLMConfig, AgentType
...
class TestAgentStatusTracking:
    ...
    async def sample_workflow(self, workflow_service):
        ...
        workflow_data = WorkflowCreate(name="Status Test Workflow", description="Workflow for testing status tracking")
    ...
    def sample_agent_data(self):
        ...
        return AgentCreate(
            ...
            llm_config=LLMConfig(provider="openai", model="gpt-4", api_key="test-key", temperature=0.7, max_tokens=1000),
            mcp_connections=[],
            max_child_agents=5,
        )
    ...
    async def test_update_status_with_description(self, agent_service, sample_agent):
        ...
        status_update = AgentStatusUpdate(status=AgentStatusEnum.RUNNING, description=custom_description)
    ...
    async def test_update_status_with_default_description(self, agent_service, sample_agent):
        ...
        status_update = AgentStatusUpdate(status=AgentStatusEnum.ERROR, description=None)
    ...
    async def test_error_status_with_details(self, agent_service, sample_agent):
        ...
        status_update = AgentStatusUpdate(status=AgentStatusEnum.ERROR, description=error_description)
    ...
    async def test_completed_status_with_summary(self, agent_service, sample_agent):
        ...
        status_update = AgentStatusUpdate(status=AgentStatusEnum.COMPLETED, description=completion_summary)
    ...
    async def test_running_status_with_current_task(self, agent_service, sample_agent):
        ...
        status_update = AgentStatusUpdate(status=AgentStatusEnum.RUNNING, description=task_description)
    ...
    async def test_paused_status_with_reason(self, agent_service, sample_agent):
        ...
        status_update = AgentStatusUpdate(status=AgentStatusEnum.PAUSED, description=pause_reason)
    ...
    async def test_update_nonexistent_agent_status(self, agent_service):
        ...
        status_update = AgentStatusUpdate(status=AgentStatusEnum.RUNNING, description="This should fail")
    ...
    async def test_backward_compatibility_method(self, agent_service, sample_agent):
        ...
        result = await agent_service.update_agent_status_simple(sample_agent.id, AgentStatusEnum.RUNNING)
    ...
    async def test_status_timeline_tracking(self, agent_service, sample_agent):
        ...
        # First update
        status_update_1 = AgentStatusUpdate(status=AgentStatusEnum.RUNNING, description="Starting task processing")
        ...
        status_update_2 = AgentStatusUpdate(
            status=AgentStatusEnum.COMPLETED, description="Task processing completed successfully"
        )
    ...
    async def test_status_persistence(self, agent_service, sample_agent):
        ...
        status_update = AgentStatusUpdate(
            status=AgentStatusEnum.ERROR, description="Database connection timeout after 30 seconds"
        )
    ...
    async def test_multiple_agents_status_independence(self, agent_service, sample_workflow, sample_agent_data):
        ...
        status_update_1 = AgentStatusUpdate(status=AgentStatusEnum.RUNNING, description="Agent 1 processing tasks")
        await agent_service.update_agent_status(agent1.id, status_update_1)

        # Update agent2 status
        status_update_2 = AgentStatusUpdate(status=AgentStatusEnum.ERROR, description="Agent 2 encountered error")
