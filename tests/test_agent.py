...
async def sample_workflow(workflow_service):
    ...
    workflow_data = WorkflowCreate(name="Test Workflow", description="A test workflow for agent testing")
...
def sample_llm_config():
    ...
    return LLMConfig(
        provider="openai", model="gpt-4", temperature=0.7, max_tokens=1000, api_key="test-key", additional_config={}
    )
...
class TestAgentService:
    ...
    async def test_connect_agents(self, agent_service, sample_workflow, sample_agent_data):
        ...
        agent2_data = sample_agent_data.model_copy()
    ...
    async def test_spawn_child_agent(self, agent_service, sample_workflow, sample_agent_data):
        ...
        child_data = sample_agent_data.model_copy()
    ...
    async def test_update_agent_status(self, agent_service, sample_workflow, sample_agent_data):
        ...
        # Update status
        status_update = AgentStatusUpdate(status=AgentStatusEnum.RUNNING, description="Running test task")
        result = await agent_service.update_agent_status(agent.id, status_update)
        assert result is not None
        assert result.status == AgentStatusEnum.RUNNING
        assert result.status_description == "Running test task"
