...
from ..models.schemas import WorkflowPlan, TaskExecution, TaskStatus, IntelligenceLevel, AgentType, AgentStatusEnum
...
class LangGraphService(LoggerMixin):
    ...
    async def execution_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            # Store result
            state["results"][step_id] = {"status": "completed", "result": result, "timestamp": datetime.now().isoformat()}
            ...
            state["results"][step_id] = {"status": "failed", "error": str(e), "timestamp": datetime.now().isoformat()}
    ...
    async def create_workflow_from_request(
        self, request: str, intelligence_level: IntelligenceLevel = IntelligenceLevel.BASIC
    ) -> str:
        ...
        workflow.add_conditional_edges(
            "execute", self.decision_node, {"execute": "execute", "recovery": "recovery", "complete": "complete"}
        )
